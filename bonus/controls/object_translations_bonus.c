...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   object_translations_bonus.c                        :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: itaharbo <itaharbo@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/08/15 17:23:51 by itaharbo          #+#    #+#             */
/*   Updated: 2025/08/28 21:09:25 by itaharbo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	cone_translation(t_minirt *mini, int key, float delta)
{
	t_vector	move;
	t_cone		*cone;
	...
	if (key == LEFT)
		...
		move = scalars_mult_by_vectors(-delta, mini->ray.viewport.forward);
	cone->points = vector_to_point(
			vector_addition(point_to_vector(cone->points), move));
}
...
void	apply_object_translation(int key, t_minirt *mini, float delta)
{
	if (mini->cmd.mode == MODE_SPHERE)
		...
		cone_translation(mini, key, delta);
