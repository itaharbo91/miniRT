...
/*   Updated: 2025/08/18 19:14:13 by itaharbo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	specular_conv(char *line, t_minirt *mini, float *dst, int *i)
{
	char	*end;
	float	specular;

	specular = ft_strtof(line, &end);
	if (!ft_isdigit(end[0]) && !ft_isspace(end[0]))
		fatal_err("Invalid specular format!", mini);
	if (specular < 0.0f || specular > 500.0f)
		return (1);
	*dst = specular;
	*i += (end - line);
	return (0);
